...
{    
    ...
    for (int i = 0; i < subviewsArray.count && !found; i++) {
        ...
        if (CGRectContainsPoint(view.frame, point)) {
            ...
            if (delegate && [delegate respondsToSelector:@selector(popoverView:didSelectItem:)]) {
                UIButton* selected = (UIButton*)[labelArray objectAtIndex:i];
                [delegate popoverView:self didSelectItem:selected.titleLabel.text];
            }
...
- (void)didTapButton:(UIButton *)sender
{
    ...
    if (delegate && [delegate respondsToSelector:@selector(popoverView:didSelectItem:)]) {
        [delegate popoverView:self didSelectItem:sender.titleLabel.text];
    }
}
